...
public class J04_Transaction {
	public static void main(String[] args) {
		...
		// 기본적으로 자바에서 생성한 연결은 오토 커밋이 활성화되어 있어서 직접 트랜잭션을 제어할 수 없다.
		try (Connection conn = connector.getConnection()) {
			// 트랜잭션을 직접 제어하기 위해 오토커밋을 비활성화
			conn.setAutoCommit(false);
			
			// 1. 판매가 발생하면 매출액이 증가한다.(오늘의 매출이 조회가 되지 않는 상태라면 새로운 행을 생성한다.)
			...
			conn.commit(); // 자동 커밋이 꺼져 있기 때문에 제대로 모두 수행했다면 수동으로 커밋해줘야 한다.
		} catch (SQLException e) {
			e.printStackTrace();
		}
	}
