...
public class C08_Object {
	...
	/*
		# Object class
			...
			- Object 클래스의 메소드는 모든 클래스 내부에 이미 상속받아져서 존재하고 있다.
			- Object 클래스의 메소드는 주로 해당 클래스에 맞는 형태로 오버라이드하여 사용하게끔 설계되어 있다.
			
		# toString()
		
			- "해당 객체를 문자열로 표현한다면 어떤 모양이어야 하는가"를 정의해두는 메서드
			- 자바의 모든 객체는 toString()을 통해 문자열로 표현될 수 있다.
			- Object 클래스에 정의되어 있는 toString()의 원형은 해당 클래스의 이름과 메모리상의 주소값을 출력하게 되어 있다.
		...
		# equals(obj)
			...
			- Object 클래스에 정의되어 있는 기본 동작은 두 인스턴스가 같은 인스턴스인지를 비교하게끔 만들어져 있다.
			
		# hashcode()
		
			- "해당 객체를 유일하게 구분할 수 있는 값은 무엇인가"를 정의해두는 메서드
			- 해당 인스턴스의 지문 같은 역할을 하는 메서드
			- Object 클래스에 정의되어 있는 기본 동작은 해당 인스턴스의 주소값을 리턴하게끔 구현되어 있다.
	 */
	...
	public static void main(String[] args) {
		...
		// System.out.println()은 사실 해당 인스턴스의 toString()을 호출하여 콘솔에 출력하는 메서드이다.
		...
		// hashcode() 실행시 메모리 주소 대신 문자열 값에 기반한 숫자값이 생성된다.
...
/*
	toString(), equals(), hashcode()를 모두 활용할만한 적절한 클래스를 하나 설계해보세요.
*/
