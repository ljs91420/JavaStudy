	...
	/*
		...
		# 업 캐스팅
			...
			- 자식 클래스 인스턴스가 업캐스팅 된 상태에서 
			  오버라이딩된 메소드(자식 클래스에서 마음대로 개조한 메소드)를 사용하는 경우 
			  자식 클래스의 메소드가 실행된다.
...
//	}
	...
	public static void main(String[] args) {
		...
		// 업캐스팅 되고 나면 Dog 타입에만 존재하는 메소드인 walk()를 사용할 수 없다.
		// 하지만 Wolf 타입에도 함께 존재하는 메소드는 사용할 수 있다.
		Dog dog1 = new Dog();
		Wolf dog2 = new Dog("늑대가 됐던 개");
		
		dog1.walk();
		// dog2.walk(); // 업캐스팅 되어서 사용 불가능
		
		dog1.pet();
		dog2.pet(); // 오버라이딩된 메소드는 업캐스팅되더라도 자식 클래스의 것으로 동작한다.
		
		dog1.run();
		dog2.run();
		
		// Wolf 타입이 되었던 Dog 인스턴스를 다시 Dog로 변환하는 것은 가능하다.
		Dog d3 = (Dog)dog2;
		d3.walk();
		...
		// Wolf 타입 매개변수에는 Wolf 타입이 될 수 있는 모든 것을 전달할 수 있다.
		hunt(wolf1);
		hunt(wolf2);
		hunt(new Dog("몽실이"));
