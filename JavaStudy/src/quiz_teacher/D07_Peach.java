...
public class D07_Peach {
	public static void main(String[] args) {
		...
		peaches.add(new Peach(2800, 4.55, "A", "김씨네농장"));
		peaches.add(new Peach(3600, 2.55, "B-", "김씨네농장"));
		peaches.add(new Peach(2700, 2.56, "B+", "김씨네농장"));
		peaches.add(new Peach(3000, 1.23, "B+", "김씨네농장"));
		
		Collections.sort(peaches);
		System.out.println("#### 무게 기준 정렬(기본) ####");
		System.out.println(peaches);
		
		System.out.println("#### 농장이름순 오름차순, 등급 기준 내림차순 ####");
		Collections.sort(peaches, new Comparator<Peach>() {
			...
			public int compare(Peach p1, Peach p2) {
				...
				if (farm1.equals(farm2)) {
					...
					// 여기까지 무사히 도착했다는 것의 의미: 두 문자열이 같지도 않고 더 짧은 문자열의 길이만큼 비교했는데
		...
		Collections.sort(peaches, new Comparator<Peach>() {
			...
			public int compare(Peach p1, Peach p2) {
				...
				// Wrapper 클래스에도 각 타입의 크기를 비교할 수 있는 메서드가 미리 구현되어 있다.
				if (grade1.equals(grade2)) {
...
class Peach implements Comparable<Peach> {
	...
	public Peach(int price, double weight, String grade, String farmName) {
		this.price = price;
		this.weight = weight;
		this.grade = grade;
