...
public class D11_ReporterList {
	...
	/*
		1. 어떤 이메일 도메인(yna.co.kr, hankyung.com 등)이 몇 개씩 있는지 세어서 출력해보기
	...
	public static void main(String[] args) {
		...
		while (m.find()) {
			...
			reporterList.add(new Reporter(m.group(3), company, field, m.group(4)));
			
			// getOrDefault(key, default) : 해당 키값으로 get이 안 되면 기본값을 준다.
		...
		// companyCategoryMap = 키값으로 회사의 이름을 쓰고 값으로 리스트를 사용한다.
		HashMap<String, List<Reporter>> companyCategoryMap = new HashMap<>();
		HashMap<String, List<Reporter>> fieldCategoryMap = new HashMap<>();
		
		// reporterList : 정규표현식으로 찾은 기자들을 모두 더해놓은 리스트
		// forEach : 하나씩 꺼내면서 반복
		for (Reporter reporter : reporterList) {
			// 기자를 한 명씩 꺼내면서 해당 기자의 회사 이름을 꺼낸다.
			String companyName = reporter.getCompany();
			
			if (!companyCategoryMap.containsKey(companyName)) {
				// 첫 등록일 때(회사 이름이 키로 등록된 적이 없을 때)
				List<Reporter> list = new ArrayList<Reporter>();
				list.add(reporter);
				companyCategoryMap.put(companyName, list);
			} else {
				// 두번째 이후의 등록(이미 리스트가 만들어져 있는 상태)
				List<Reporter> list = companyCategoryMap.get(companyName);
				list.add(reporter);
			}
		}
		
		for (String company : companyCategoryMap.keySet()) {
			System.out.println("### " + company + "의 기자들 ###");
			for (Reporter reporter : companyCategoryMap.get(company)) {
		...
		for (Reporter reporter : reporterList) {
			...
			if (fieldCategoryMap.containsKey(fieldName)) {
				fieldCategoryMap.get(fieldName).add(reporter);
			} else {
				ArrayList<Reporter> list = new ArrayList<>();
				list.add(reporter);
				fieldCategoryMap.put(fieldName, list);
			}
		}
		
		for (String field : fieldCategoryMap.keySet()) {
			System.out.println("### " + field + "의 기자들 ###");
			for (Reporter reporter : fieldCategoryMap.get(field)) {
				System.out.println(reporter);
			}
		}
	}
