...
public class D07_Comparator {
	/*
		# Comparable
			...
			- compareTo() 메서드를 구현해두면 크기 비교가 필요한 상황에서 해당 메서드를 호출해 크기를 비교할 수 있게 된다.
	...
	public static void main(String[] args) {
		...
		grapes.add(new Grape("김씨네 농장", 30, 5.8, "A+"));
		grapes.add(new Grape("박씨네 농장", 28, 8.1, "B+"));
		grapes.add(new Grape("이씨네 농장", 40, 6.5, "A-"));
		grapes.add(new Grape("제갈씨네 농장", 35, 7.8, "C"));
		grapes.add(new Grape("남궁씨네 농장", 35, 5.4, "B-"));
		grapes.add(new Grape("최씨네 농장", 35, 7.7, "A"));
		grapes.add(new Grape("최씨네 농장", 20, 8.8, "B"));
		
		// sort()를 할 수 있으려면 리스트에 속한 모든 요소들의 Comparable이 구현되어 있어야 한다.
		// Collections.sort(grapes);
		
		// Comparator를 함께 전달하면 Comparable의 compareTo() 대신 Comparator의 compare()를 사용하여 정렬한다.
		...
		Collections.sort(grapes, new Comparator<Grape>() {
			...
			public int compare(Grape g1, Grape g2) {
				if (g1.sweetness == g2.sweetness) {
					return 0;
				} else if (g1.sweetness > g2.sweetness) {
					return 1;
				} else {
					return -1;
				}
...
// Comparable 인터페이스를 구현한 클래스는 각 인스턴스끼리의 크기 비교가 가능해진다.(즉, 정렬이 가능해진다.)
class Grape implements Comparable<Grape> {
	...
	public Grape(String farm, int qty, double sweetness, String grade) {
		this.farm = farm;
		this.qty = qty;
	...
	public String toString() {
		return String.format("[%s/%d알/%.1f/%s]\n", farm, qty, sweetness, grade);
	}
	
	@Override
	public int compareTo(Grape g) {
		// 현재 인스턴스와 매개변수로 전달받은 인스턴스를 비교하였을 때 어떤 것이 더 크다고 판정할 것인지 정해야 한다.
		...
		if (this.qty == g.qty) {
			...
			if (this.sweetness == g.sweetness) {
				return 0;
			} else if (this.sweetness > g.sweetness) {
				return 1;
			} else {
				return -1;
			}
...
// 내림차순 정렬 : 큰 게 먼저 나오는 것
// 오름차순 정렬 : 작은 게 먼저 나오는 것
...
/*
    ...
    연습3: 복숭아를 리스트에 넣고 등급 기준으로 내림차순 정렬하고, 등급이 같은 경우 가격 기준으로 내림차순 정렬해보세요.
