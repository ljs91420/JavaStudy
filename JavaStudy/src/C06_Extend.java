...
public class C06_Extend {
	...
	/*
		# 클래스 상속
			...
			- 자식 클래스는 부모로부터 모든 변수와 메소드를 물려받아 그대로 가지고 있다.
			- 자식 클래스는 부모로부터 물려받은 변수 및 기능을 마음대로 고쳐서 사용할 수 있다.(오버라이드, Override)
...
class Wolf {
	...
	void pet() {
		System.out.printf("늑대(%s, %d살)를 쓰다듬다가 손이 물렸습니다.\n", this.name, this.age);
	}
...
class Dog extends Wolf {
	...
	// 부모 클래스에 있는 똑같은 이름의 메소드를 자식 클래스에 만들 수 있다.
	void pet() {
		System.out.printf("강아지(%s, %d살)를 쓰다듬어줬더니 아주 좋아합니다.\n", this.name, this.age);
	}
	
	void walk() {
		System.out.printf("강아지(%s, %d살)와 함께 산책했습니다!\n", this.name, this.age);
		walkDesire -= 10;
	}
...
class Phone {
	...
	void call(String person) {
		System.out.printf("%s에게 전화를 걸었습니다.\n", person);
	}
}

class SmartPhone extends Phone {
	
	void OperateApp(String app) {
		System.out.printf("%s 애플리케이션을 동작시켰습니다.\n", app);
	}
}
...
class PurchaseBuilding extends Building {
	...
	// 하지만, 기본 생성자는 생략이 가능하기 때문에 안 적는 것처럼 보이는 경우도 있다.
