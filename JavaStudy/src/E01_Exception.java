...
public class E01_Exception {
	...
	/*
		...
		# 예외 처리(try-catch문)
			...
			- 예외가 발생했을 때 예외 처리가 되어 있지 않은 경우 프로그램이 강제 종료되지만
			  해당 예외에 대한 처리가 되어 있는 경우 프로그램이 강제 종료되지 않는다.
			- 예외가 발생하여 해당 catch문에 도착할 때 발생한 예외에 대한 정보를 담고 있는 인스턴스가 함께 도착한다.
			- 예외 처리를 하지 않았을 때의 기본 동작은 e.printStackTrace() 후 강제 종료이다.
			
		# finally
		
			- try문 내부에서 예외가 발생하든 안 하든 무조건 실행하게 되는 문법
	 */
	...
	public static void main(String[] args) {
		try {
			...
			// 예외가 발생했을 당시의 스택 상황을 출력해주는 메서드 (예외 처리 기본 동작 1)
			e.printStackTrace(); 
			// 프로그램을 강제로 종료한다.(예외 처리 기본 동작 2)
			System.exit(-1);
		}
		...
		try {
			...
			// Exception은 모든 예외 객체의 조상 클래스이기 때문에 모든 예외가 이 catch문으로 도착할 수 있다.
			System.out.println("예외가 발생했습니다!");
		} finally {
			// try문 내부에서의 예외 발생 여부와 관계 없이 무조건 실행하고 싶은 코드를 finally에 적는다.
			System.out.println(" /)/)");
			System.out.println("(  ..)");
			System.out.println("(  >$");
		}
